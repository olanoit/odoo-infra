...
./scripts/ops.sh module \
  odoo19_motomarket_sta \
  motomarket_sta_principal \
  "extendrix_field_service_motomarket,extendrix_biometricos" \
  update
...
# Ejemplo real:
docker exec -it odoo19_motomarket_sta odoo \
  --config=/etc/odoo/odoo.conf \
  --database=motomarket_sta_principal \
  --update=extendrix_field_service_motomarket \
  --stop-after-init
```
...
nano projects/motomarket/odoo19/sta/addons/extendrix_field_service_motomarket/models/mi_modelo.py
...
mkdir -p projects/motomarket/odoo19/sta/addons/extendrix_nuevo_modulo

# 2. Crear __manifest__.py y __init__.py mínimos
cat > projects/motomarket/odoo19/sta/addons/extendrix_nuevo_modulo/__manifest__.py << 'EOF'
{
    'name': 'Extendrix - Nuevo Módulo',
    'version': '19.0.1.0.0',
...
cat > projects/motomarket/odoo19/sta/addons/extendrix_nuevo_modulo/__init__.py << 'EOF'
# -*- coding: utf-8 -*-
EOF
...
# Ver si levantó bien
docker compose ps odoo19_motomarket_sta
./scripts/ops.sh logs odoo19_motomarket_sta 30
```
...
./scripts/ops.sh logs odoo19_motomarket_sta 0
# (0 = sin límite de líneas, solo lo nuevo)

# Filtrar solo los errores:
docker logs -f odoo19_motomarket_sta 2>&1 | grep -E "ERROR|WARNING|Traceback"
```
...
# https://motomarket-sta.extendrix.work/web?debug=1
...
./scripts/ops.sh module odoo19_motomarket_sta motomarket_sta_principal modulo_dependencia install
# Luego tu módulo
...
./scripts/ops.sh db-query odoo19_motomarket_sta motomarket_sta_principal \
  "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='motomarket_sta_principal' AND pid <> pg_backend_pid();"
