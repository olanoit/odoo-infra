...
docker compose logs -f odoo18_farmaniacos_sta

# Últimas N líneas + seguir en tiempo real
./scripts/ops.sh logs odoo18_farmaniacos_sta 200

# Solo errores
docker logs odoo18_farmaniacos_sta 2>&1 | grep -E "ERROR|CRITICAL|Traceback"
...
./scripts/ops.sh module odoo18_farmaniacos_sta farmaniacos_sta_principal "modulo1,modulo2" update
```
...
docker exec -it odoo18_farmaniacos_sta bash

# Ejecutar un comando Python en el contexto de Odoo (scaffold, etc.)
docker exec -it odoo18_farmaniacos_sta python3 -c "import odoo; print(odoo.__version__)"
...
./scripts/ops.sh update-image odoo18_farmaniacos_sta
...
echo | openssl s_client -connect farmaniacos-sta.altalatam.com:443 2>/dev/null \
...
docker compose stop odoo18_farmaniacos_sta
docker compose rm -f odoo18_farmaniacos_sta
docker compose up -d odoo18_farmaniacos_sta
