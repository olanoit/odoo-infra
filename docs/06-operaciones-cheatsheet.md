...
docker compose logs -f odoo14_merida_sta

# Últimas N líneas + seguir en tiempo real
./scripts/ops.sh logs odoo14_merida_sta 200

# Solo errores
docker logs odoo14_merida_sta 2>&1 | grep -E "ERROR|CRITICAL|Traceback"
...
./scripts/clean-logs.sh odoo14_merida_sta
...
./scripts/ops.sh module odoo14_merida_sta merida_sta_principal "modulo1,modulo2" update
```
...
docker exec -it odoo14_merida_sta bash

# Ejecutar un comando Python en el contexto de Odoo (scaffold, etc.)
docker exec -it odoo14_merida_sta python3 -c "import odoo; print(odoo.__version__)"

# Ejecutar query SQL directamente en PostgreSQL
docker exec -it odoo_postgres psql -U odoo -d merida_sta_principal -c "SELECT version();"

# Acceder a psql interactivo de una DB
docker exec -it odoo_postgres psql -U odoo -d merida_sta_principal
```
...
docker exec odoo_postgres createdb -U odoo merida_sta_pruebas

# Eliminar una DB (⚠️ irreversible)
docker exec odoo_postgres dropdb -U odoo merida_sta_pruebas
...
./scripts/ops.sh update-image odoo14_merida_sta
...
echo | openssl s_client -connect merida-sta.odoo-rideco.mx:443 2>/dev/null \
...
```bash
# Backup completo (DB + filestore) → ./backups/merida/
...
docker compose stop odoo14_merida_sta
docker compose rm -f odoo14_merida_sta
